...
function requireReact_reactServer_production() {
  ...
  react_reactServer_production.version = "19.2.8";
...
function requireReactDom_reactServer_production() {
  ...
  reactDom_reactServer_production.version = "19.2.8";
...
function requireReactServerDomWebpackServer_edge_production() {
  ...
  function serializeAsyncIterable(request, task, iterable, iterator) {
    ...
    function error(reason) {
      0 === streamTask.status && (request.cacheController.signal.removeEventListener(
        "abort",
        abortIterable
      ), erroredTask(request, streamTask, reason), enqueueFlush(request), "function" === typeof iterator.throw && iterator.throw(reason).then(noop, noop));
    }
  ...
  reactServerDomWebpackServer_edge_production.decodeAction = function(body, serverManifest) {
    ...
    if (actionKey.startsWith("$ACTION_REF_"))
      actionKey = "$ACTION_" + actionKey.slice(12) + ":", body = decodeBoundActionMetaData(body, serverManifest, actionKey), action = loadServerReference(serverManifest, body);
    else if (actionKey.startsWith("$ACTION_ID_"))
      body = actionKey.slice(11), action = loadServerReference(serverManifest, { id: body, bound: null });
  ...
  globalThis.__vite_rsc_server_require__ = memoize(async (id) => {
    if (id.startsWith("$$preserve:")) {
      ...
      const getOrCreateServerReference = (name) => {
        return target[name] ??= server_edgeExports.registerServerReference(() => {
...
function renderToReadableStream(data, options, extraOptions) {
  return renderToReadableStream$1(data, options);
}
  ...
  try {
    ...
    if (action === "move") {
      ...
      if (target !== color) {
        ...
        if (target === "red") {
          await db.prepare("UPDATE rooms SET black_token = NULL, black_name = NULL, black_seen = NULL, red_token = ?, red_name = ?, red_seen = ?, revision = revision + 1, updated_at = ? WHERE room_id = ?").bind(token, name, now, now, roomId).run();
        } else {
          await db.prepare("UPDATE rooms SET red_token = NULL, red_name = NULL, red_seen = NULL, black_token = ?, black_name = ?, black_seen = ?, revision = revision + 1, updated_at = ? WHERE room_id = ?").bind(token, name, now, now, roomId).run();
        }
      }
...
function RootLayout({ children }) {
  return /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("html", { lang: "zh-Hant", children: /* @__PURE__ */ jsxRuntime_reactServerExports.jsx("body", { style: { margin: 0 }, children }) });
}
