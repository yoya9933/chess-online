...
- 正式觀戰、單機 AI 3.2.1：Iterative Deepening + Alpha-Beta + Transposition Table + Quiescence Search + Move Ordering + Piece-Square Tables + Piece-directed Move Generator + Opening Book
- AI 三段難度搜尋上限為簡單 2 ply、普通 6 ply、困難 9 ply；普通／困難思考時間仍維持 320ms／850ms，以更快的走法生成和低配置棋盤複製換取更高完成深度
- 標準象棋 Opening Book：開局階段依中炮、屏風馬、仙人指路、飛相等常見著法加權選擇；同一組開局著法即使順序不同或中間插入其他合法發展著，也能用 transposition matching 重新接回相同定式。AI 會記住各開局節點上一局的選擇，簡單／普通避免連續重複同一著，困難則降低重複著權重以兼顧棋力與變化
...
AI 搜尋統計可由瀏覽器執行 `ChuhePlatform.ai.lastSearch` 查看；最近一次 AI 決策來源可由 `ChuhePlatform.ai.lastDecision` 查看，會標示 `opening-book`、`search` 或 `perpetual-check-guard`，Opening Book 命中時也會標示 `sequence` 或 `transposition`。
