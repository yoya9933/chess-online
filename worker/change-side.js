...
export async function handleChangeSide(request, env) {
  try {
    ...
    let room = await env.DB.prepare("SELECT * FROM rooms WHERE room_id = ?").bind(roomId).first();
    if (!room) return json({ error: "找不到房間" }, 404);

    const color = playerColor(room, token);
    if (color === "spectator") return json({ error: "觀戰者不能切換陣營" }, 403);
    if (target === color) return json({ color, players: players(room), revision: room.revision, swapped: false });
    ...
    const targetActive = targetOccupied && now - targetSeen <= STALE_AFTER_MS;
    let result;
    ...
    room = await env.DB.prepare("SELECT * FROM rooms WHERE room_id = ?").bind(roomId).first();
    return json({
      color: playerColor(room, token),
      players: players(room),
      revision: room.revision,
      swapped: targetActive,
    });
  } catch (error) {
    console.error(error);
